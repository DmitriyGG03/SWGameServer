...
public class MapService : IMapService
{
    ...
    public async Task<ServiceResult<SessionMap>> GetMapAsync(Guid sessionId, MapGenerationOptions options, CancellationToken cancellationToken)
    {
        var hero = await _context.Heroes.FirstOrDefaultAsync(x => x.SessionId == sessionId, cancellationToken);
        if (hero == null)
        {
        ...
        return new ServiceResult<SessionMap>("There is no map in session");
