...
public class MapGeneratorService
{
        ...
        private List<Planet> GeneratePlanets(MapGenerationOptions options)
        {
            ...
            {
                PointF position = Geometry.GenerateRandomPoint(options.Width, options.Height);
                // Calculate distance
                while (planets.Any(p => Geometry.CalculateDistance(p.Position, position) < options.MinDistanceBetweenPlanets))
                {
            ...
            foreach (Planet planet in planets)
            {
                ...
                foreach (Planet neighbor in neighbors)
                {
                    ...
                    if (!overlaps)
                    {
                        ...
                        bool tooClose = IsConnectionTooCloseToSomePlanet(connection, planet, neighbor, planets, options.MinDistanceFromPlanetToEdge);
                        if (!tooClose)
                        {
        ...
        private bool IsConnectionTooCloseToSomePlanet(Edge connectionToCheck, Planet currentPlanet, Planet neighbor, List<Planet> planets, int minDistanceFromPlanetToEdge)
        {
            bool tooClose = false;
            foreach (Planet otherPlanet in planets.Where(p => p != currentPlanet && p != neighbor))
            {
                if (Geometry.CalculateDistanceToSegment(connectionToCheck.From.Position, connectionToCheck.To.Position, otherPlanet.Position) < minDistanceFromPlanetToEdge)
                {
                    tooClose = true;
                    break;
                }
        ...
        private bool IsConnectionOverlapsOthers(Edge connectionToCheck, List<Edge> connections)
        {
            ...
            foreach (var existingConnection in connections)
            {
                ...
                if (Geometry.Intersects(connectionToCheck.From.Position, connectionToCheck.To.Position, existingConnection.From.Position, existingConnection.To.Position))
                {
                    overlaps = true;
                    break;
                }
